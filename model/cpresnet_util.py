...
from model.classifier import ConvBnClassifier
...
def get_default_config(rho=8, in_channels=1, arch="cp_resnet", n_classes=13,
...
class CPResNet(_BaseBackbone):
    def __init__(self, config=None):
        ...
        self.classifier = ConvBnClassifier(n_channels[2], n_classes)
...
def get_model(rho=8, in_channels=1, arch="cp_resnet", n_classes=13,
              base_channels=32, cut_channels_s2=0, cut_channels_s3=36, channels_multiplier=2, n_blocks=(2, 1, 1),
              s1_group=1, s2_group=2, s3_group=1,
              maxpool_pos_stage1=None, maxpool_pos_stage2=None, maxpool_pos_stage3=None,
              maxpool_stage1_pos0_kernel=(2, 2), maxpool_stage1_pos0_stride=(2, 2),
    ...
    m = CPResNet(model_config)
    return m


class CPResNetLoadCheckpoint(_BaseBackbone):
    def __init__(self, ckpt_path,  num_classes: int = 10):
        super(CPResNetLoadCheckpoint, self).__init__()
        prefix = 'backbone.'
        ...
        ff_list = []
        ff_list += [nn.Conv2d(
            model.classifier[0].in_channels,
            num_classes,
            kernel_size=1,
            stride=1,
            padding=0,
        ...
        # initialize weights
        if isinstance(self.classifier[0], nn.Conv2d):
            self.classifier[0].weight.data.zero_()
