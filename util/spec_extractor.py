...
class BEATsMel(_SpecExtractor):
    ...
    def forward(self, x):
        # Move to CPU once for kaldi processing
        device = x.device
        if device.type == 'mps':
            x = x.cpu()
        ...
        for waveform in x:
            ...
            fbank = ta_kaldi.fbank(waveform, num_mel_bins=128, sample_frequency=16000,
...
class PaSSTMel(_SpecExtractor):
    def __init__(self, n_mels=128, sr=32000, win_length=800, hopsize=320, n_fft=1024, freqm=48, timem=20,
                 fmin=0.0, fmax=None, fmin_aug_range=1, fmax_aug_range=1000):
        """
