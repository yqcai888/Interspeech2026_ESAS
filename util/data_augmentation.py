...
class MixUpMultiLabels(_DataAugmentation):
    def __init__(self, alpha=0.3):
        super(MixUpMultiLabels, self).__init__()
        self.alpha = alpha
        self.lam = 1
    ...
    def cal_loss(self, y_hat, y, loss_func):
...
class DeviceImpulseResponseAugmentation(_DataAugmentation):
    ...
    def forward(self, x, d, sampling_rate=32000):
        batch_size = x.size(0)
        for i in range(batch_size):
            ...
            if d[i] == torch.tensor([0]).to(x.device) and np.random.uniform(0, 1) < self.p:
                ...
                ir, _ = librosa.load(f"{self.path_ir}/{random_file}", sr=sampling_rate)
